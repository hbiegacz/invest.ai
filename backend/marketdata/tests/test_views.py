...
def test_binance_test_view_returns_success(api_client, mocker):
    mocker.patch("marketdata.services.BinanceAPIService.BinanceAPIService.get_btc_price", return_value={"price": "50000"})
...
def test_naive_model_view_returns_prediction(api_client, mocker):
    mocker.patch("marketdata.services.ModelService.ModelService.naive", return_value=50000.0)
...
def test_linear_regression_model_view_handles_missing_file(api_client, mocker):
    mocker.patch("marketdata.services.ModelService.ModelService.linear_regression", side_effect=FileNotFoundError("Model not found"))
