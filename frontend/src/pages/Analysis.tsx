...
function BinanceCandleChart({
  ...
  const showPrediction =
    predictionPoint &&
    Number.isFinite(predictionPoint.x) &&
    Number.isFinite(predictionPoint.y);
    ...
    <div className="rounded-2xl border bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h4 className="font-medium">
          Drugi wykres (dowolny parametr z endpointu)
        </h4>
        <select
          className="rounded-xl border px-3 py-1 text-sm bg-white"
          value={metricKey}
...
export default function Analysis() {
  ...
  const [selectedModel, setSelectedModel] =
    React.useState<ModelKey>("baseline");
  const [syncZoom, setSyncZoom] = React.useState(true);

  const [modelLoading, setModelLoading] = React.useState(false);
  const [modelError, setModelError] = React.useState<string | null>(null);
  const [predictionPoint, setPredictionPoint] =
    React.useState<MetricPoint | null>(null);
  const [predictedClose, setPredictedClose] = React.useState<number | null>(
    null
  );
  ...
  const metricSeries = React.useMemo(
    () => buildMetricSeries(rows, metricKey),
    [rows, metricKey]
  );
  ...
  async function handleRunModel() {
    ...
    try {
      if (!rows.length)
        throw new Error(
          "Brak danych historycznych – nie mogę osadzić punktu predykcji na osi czasu."
        );
  ...
  return (
    <section className="grid gap-6">
      ...
      {!loading && !error && rows.length === 0 && (
        <p className="text-sm text-slate-500">
          Brak danych z endpointu historical-data.
        </p>
      )}
      ...
      {!loading && !error && rows.length > 0 && (
        <div className="grid gap-6">
          ...
          <MetricBarChart
            series={metricSeries}
            metricKey={metricKey}
            zoomOptions={metricZoomOptions}
            chartRef={metricChartRef}
